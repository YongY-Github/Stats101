...
10. Fifty students from GA met to form a club. Fifteen students wore glasses, 18 are left-handed and 21 are female. There are four females who are left-handed, three females wear glasses, and five members who wear glasses and are left-handed. Only one member wers glasses, is left-haded and female.

  - Draw a Venn diagram to represent these data.
  - A member is selected at random. Find the probabilty that the member
  (i) is male, does not wear glasses and is not left-handed.
  (ii) wears glasses given that she isleft-handed and female.

------------------------------------------------------------------------

### Multiplication rule and independence

11. **Without replacement**\
...
14. The events A and B are such that $P(A) = 1/3$, $P(B) = 1/4$ and $P(A \cup B) = 1/2$.
...
15. **Addiitonal rule**\
    ...
    $P(X)=1/5$, $P(Y)=1/4$, and $P(X| Y )= 2/5$.

    -   Find $P(X \text{ or } Y)$

14. **Mutually exclusive or not?**\
...
16. Suppose that 6 balls are randomly thrown into 10 baskets.
    Find the probabilties that:
...
17. Three students from GA enter a mini-marathon. The probabilities that each will complete the marathon are $0.9, 0.7$ and $0.6$.
    Assuming that their performance are independent, find the probability that:
...
21. Set operations and De Morgan’s laws
